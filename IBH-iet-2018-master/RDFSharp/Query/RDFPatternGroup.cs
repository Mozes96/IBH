...
namespace RDFSharp.Query
{
    ...
    public class RDFPatternGroup: IEquatable<RDFPatternGroup> {
        ...
        public RDFPatternGroup AddPattern(RDFPattern pattern)
        {
            ...
            if (pattern != null && pattern.Variables.Count > 0 && !this.Patterns.Exists(p => p.Equals(pattern)))
            {
                ...
                if (pattern.Context != null && pattern.Context is RDFVariable &&
                    !this.Variables.Exists(v => v.Equals(pattern.Context)))
                    this.Variables.Add((RDFVariable) pattern.Context);

                //Subject
                if (pattern.Subject is RDFVariable && !this.Variables.Exists(v => v.Equals(pattern.Subject)))
                    this.Variables.Add((RDFVariable) pattern.Subject);

                //Predicate
                if (pattern.Predicate is RDFVariable && !this.Variables.Exists(v => v.Equals(pattern.Predicate)))
                    this.Variables.Add((RDFVariable) pattern.Predicate);

                //Object
                if (pattern.Object is RDFVariable && !this.Variables.Exists(v => v.Equals(pattern.Object)))
        ...
        public RDFPatternGroup AddFilter(RDFFilter filter) {
            if (filter != null) {
                if (this.Filters.Exists(f => f.Equals(filter))) return this;
